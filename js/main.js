...
document.addEventListener('DOMContentLoaded', function() {
    console.log('ASUS Store - Vanilla JS Version');
...
function updateCheckoutSummary() {
    ...
    if (checkoutItems && items.length > 0) {
        ...
        items.forEach(item => {
            ...
            itemEl.innerHTML = `
                ...
                <span class="summary-value">$${(item.product.price * item.quantity).toLocaleString()}</span>
            `;
    ...
    if (checkoutSubtotal) checkoutSubtotal.textContent = `$${totalPrice.toLocaleString()}`;
...
window.addEventListener('popstate', function(e) {
    // Reload page content when using browser navigation
    location.reload();
});
...
console.log('%c ASUS Store - Vanilla JS Version ', 'background: #3d5a80; color: #fff; padding: 5px 10px; border-radius: 3px;');
console.log('%c In Search of Incredible ', 'color: #3d5a80; font-style: italic;');
